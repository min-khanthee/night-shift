...
// Dims selected lines if not dimmed, or undims them if already dimmed.
  const toggleDim = vscode.commands.registerCommand('night-shift.toggleDim', () => {
...
const isolateSelection = vscode.commands.registerCommand('night-shift.isolateSelection', () => {
  const editor = vscode.window.activeTextEditor;
  if (!editor) {
    return vscode.window.showInformationMessage('No active editor');
  }
...
// Undims all lines in the active file and removes them from memory
  const clearDimsInFile = vscode.commands.registerCommand('night-shift.clearDimsInFile', () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      return vscode.window.showInformationMessage('No active editor');
    }
...
// Completely removes all dimmed lines across every file and editor window
  const clearAllDimsCommand = vscode.commands.registerCommand('night-shift.clearAllDims', async () => {
...
void Promise.resolve().then(() => applyDims(vscode.window.activeTextEditor));
